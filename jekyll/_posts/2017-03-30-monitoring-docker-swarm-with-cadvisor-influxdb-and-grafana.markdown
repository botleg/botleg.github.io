...
* `Docker`: version >= 17.10 CE, to support Docker Compose File version 3 and Swarm Mode.
* `Docker Machine`: version >= 0.12
* `Docker Compose`: version >= 1.16, to support Docker Compose file version 3
...
cAdvisor service has much more configuration required than the other services. To know more, check [this](https://github.com/google/cadvisor) out. The hostname key is a tricky one. We intend to put one agent in each node of the swarm and this container will collect all metrics from the node and the containers running in it. When cAdvisor send metrics to InfluxDB, it send it with a tag `machine` that contains the hostname of cAdvisor container. We need to match it with the hostname of the node running it. Docker stacks allows templating in naming. We have named the containers with the ID of the node running it so that we know where the metric is coming from. The is done by the value `'{{"{{"}}.Node.Hostname}}'`.
